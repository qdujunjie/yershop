...
<body>
	...
	<div class="main-title">
		<h2>优惠券管理 [ <?php if(isset($_GET['group'])): ?><a href="<?php echo U('index');?>">全部</a><?php else: ?><strong>全部</strong><?php endif; ?>&nbsp;<?php if(is_array($group)): foreach($group as $key=>$vo): if(($group_id) != $key): ?><a href="<?php echo U('index?group='.$key);?>"><?php echo ($vo); ?></a><?php else: ?><strong><?php echo ($vo); ?></strong><?php endif; ?>&nbsp;<?php endforeach; endif; ?> ]</h2>
	</div>
	...
	<div class="cf">
        ...
        <div class="search-form fr cf">
            <div class="sleft">
                <input type="text" name="title" class="search-input" value="<?php echo I('title');?>" placeholder="请输入优惠券名称">
                <a class="sch-btn" href="javascript:;" id="search" url="/admin.php?s=/Fcoupon/index.html"><i class="btn-search"></i></a>
            </div>
        </div>
	</div>
				...
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$order): $mod = ($i % 2 );++$i;?><tr>
					<td><input class="ids row-selected" type="checkbox" name="id[]" value="<?php echo ($order["id"]); ?>"></td>
                        ...
                        <td><?php echo ($order["price"]); ?></td>
						 ...
						 <td><img src="<?php echo (get_cover($order["icon"],'path')); ?>"  width="50" height="50"/></td>
