...
<dd><a rel="nofollow" href='<?php echo U("center/coupon");?>'>优惠券</dd><dd><a rel="nofollow" href='<?php echo U("center/allorder");?>'>我的全部订单</a></dd><dd><a rel="nofollow" href='<?php echo U("center/unfinished");?>'>未完成订单</a></dd><dd><a rel="nofollow" href='<?php echo U("center/sended");?>'>已发货订单</a></dd>
...
<span  style="display: block;padding:15px;font-size:17px;text-align:center;color:red;margin:20px 0;"><?php echo ($msg); ?> &nbsp;&nbsp;<?php echo ($id); ?>反馈</span>
<span  style="display: block;padding:15px;font-size:17px;text-align:left;color:red;margin:5px 0;">当前状态： &nbsp;&nbsp;<?php echo ($backinfo); ?></span>
	<form action='<?php echo U();?>' method="post" name="actform" style="float:left;;width:10%" >
	
       </form>
<div style="display: block;float:right;width:88%">     
<?php if(empty($list)): ?><div class="personal_info_none_goods">
	...
	<?php else: ?>
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><p style="display: block;padding:15px;margin:10px;color:white;background: #64AADB">反馈时间：<?php echo (date('Y-m-d',$vo["update_time"])); ?></p>  
            <p><?php echo ($vo["info"]); ?></p>
        
      	<p style="display: block;padding:15px;margin:10px;color:white;background:#64AADB">申请时间：<?php echo (date('Y-m-d',$vo["time"])); ?></p>
 <p>标题<?php echo ($vo["title"]); ?></p>
        
 <p>原因：<?php echo ($vo["reason"]); ?></p><?php endforeach; endif; else: echo "" ;endif; endif; ?>
