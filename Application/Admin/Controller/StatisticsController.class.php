...
$data["status"]='1';
     ...
     $t_time=$turnover->where("status='1'")->order("id desc")->limit(1)->getField("time");
     /*格式时间戳为 20141024*/
	 $r=date('Ymd',$t_time);/*格式时间戳为 20141024*/
	 if($q==$r){
		 /*当天已统计过数据，保存，创建时间不变，更新时间变化*/
     $data["update_time"]=NOW_TIME;
	 $turnover->where("time='$t_time' and status='1'")->save($data);
	 }
  else{ 
	  /*未统计过数据，新增*/
   $data["time"]=NOW_TIME;
 ...
 /**
   ...
   /*利润*/
	...
	$this->assign('profits',$profits);
	 ...
	 $data["info"]=$y;
     $t_time=$turnover->where("status='2'")->order("id desc")->limit(1)->getField("time");
     /*格式时间戳为 20141024*/
	 $r=date('W',$t_time);/*本年度的第几周*/
	 if($y==$r){
		 /*本周已统计过数据，保存，创建时间不变，更新时间变化*/
     $data["update_time"]=NOW_TIME;
	 $turnover->where("time='$t_time' and status='2'")->save($data);
	 }
  else{ 
	  /*未统计过数据，新增*/
   $data["status"]='2';
...
$endThismonth=mktime(23,59,59,date('m'),date('t'),date('Y'));
  ...
  /**
   ...
   /*利润*/
    ...
    $data["info"]=$t;
     $sqtime=$turnover->where("status='3'")->order("id desc")->limit(1)->getField("time");
     /*格式时间戳为 20141024*/
	 ...
	 if($t==$r){
		 /*当天已统计过数据，保存，创建时间不变，更新时间变化*/
     $data["update_time"]=NOW_TIME;
	 $turnover->where("time='$sqtime' and status='3'")->save($data);
	 }
  else{ 
	  /*未统计过数据，新增*/
$data["status"]='3';
