		...
		<div class="pi_my">
			...
			<div class="pi_my_div">
				<div class="pi_my_div_top">
					<a href="https://vip.wps.cn/center/#!/center/" class="pi_my_name" title="云云1412694646"><?php echo (get_nickname($uid)); ?></a>
					<span class="qingtong"></span><span class="dujia" style="display:none;"></span>
					<span class="grade_text"><?php echo (get_lever($uid)); ?></span>
...
<a rel="nofollow" href="<?php echo U("center/coupon");?>" class="pi_food_btn" >优惠券<span class="red"><?php echo ((isset($num) && ($num !== ""))?($num):"0"); ?></span>张</a>
					<a rel="nofollow" href="<?php echo U("center/coupon");?>" class="pi_food_btn" >免费领优惠券</a>
				</div>
			

			<div class="pi_food_div pi_food_div2">
				<p class="pi_food_price">
					我的积分<span class="red" id="score"><?php echo (get_score($uid)); ?></span>点				
				</p>							
			</div>
			<div class="cb"></div>
	...
	<?php if(empty($lateorder)): ?><div class="personal_info_none_goods">
				<p>你的订单还是空的哦<br>赶快行动吧！</p>
				<a rel="nofollow" href="/latest/" class="ping_btn">马上购物</a>
				<div class="cb"></div>
			</div>
	<?php else: ?>
       
    
          <form action='<?php echo U("Shopcart/delorder");?>' method="post" name="delform" >
       <div class="order_del">
			  <span>  <input class="checkbox check-all" type="checkbox"> 全选 <a   href='javascript:vod(0)' onclick="delorder()">删除选中的订单</a> </span></div>
          <?php if(is_array($lateorder)): $i = 0; $__LIST__ = $lateorder;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$po): $mod = ($i % 2 );++$i;?><table  id="table" class="gridtable" width="100%">
        <thead>
            <tr><th class="row-selected">
				...
				<th ><a href="<?php echo U('order/detail?id='.$po['orderid']);?>" >订单详情</a></th>
                <th > <?php echo ($po["ptime"]); ?></th>
               
                <th class="info"><?php echo ($po["backinfo"]); ?></th>
            </tr>
        </thead>
        ...
        <tbody> <?php if(is_array($po['id'])): $i = 0; $__LIST__ = $po['id'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr><td width="40"></td>
                ...
                <td><?php echo (get_good_price($vo["goodid"])); ?></td>
            </tr><?php endforeach; endif; else: echo "" ;endif; ?>
		
			  <tr><td colspan="5" align="right">金额小计：<span class="red"><?php echo ($po["pricetotal"]); ?></span>元(运费<span class="red"><?php echo ($po["shipprice"]); ?></span>元)<?php $pay=$po['ispay']; if($pay==1){ echo "<A class='red' href='http://".$_SERVER['HTTP_HOST']."/index.php?s=/Home/Pay/index/orderid/".$po['orderid']."'>前去支付</a>";}; ?>  </td>
			</tr>
	...
	<div style="display: block;" class="tab_view">
		<div id="my_shopping_car" style="display: block;">
			<?php if(empty($cartlist)): ?><div class="personal_info_none_goods">
				<p>你的购物车还是空的哦<br>赶快行动吧！</p>
				<a rel="nofollow" href="<?php echo U("shopcart/index");?>" class="ping_btn">马上购物</a>
				<div class="cb"></div>
			</div>
	<?php else: ?>
	...
	<table  id="table" class="gridtable" width="100%">
        ...
        <tbody>
          ...
          <?php if(is_array($cartlist)): $i = 0; $__LIST__ = $cartlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td ><a rel="jia"  class="jia" onclick="myfunction(event)">+</a><input type="text"  class="num"  id="<?php echo ($vo["goodid"]); ?>" value="<?php echo ($vo["num"]); ?>"/><a rel="jian" onclick="myfunction(event)" class="jian" id="oneA">-</a></td>
                
                <td><a  name="<?php echo ($vo["goodid"]); ?>" rel="del" onclick="myfunction(event)">删除</a>&nbsp;<a href="#">移到收藏</a></td>
            </tr><?php endforeach; endif; else: echo "" ;endif; ?>
		...
		<?php if(empty($favorlist)): ?><div class="personal_info_none_goods">
			<p>你的收藏列表还是空的哦<br>赶快行动吧！</p>
			<a rel="nofollow" href="http://www.docer.com" class="ping_btn">马上收藏</a>
			<div class="cb"></div>
		
		</div>
		<?php else: ?>
		<ul> <?php if(is_array($favorlist)): $i = 0; $__LIST__ = $favorlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li><span><A href="<?php echo U('Article/detail?id='.$vo['goodid']);?>" > <img src="<?php echo (get_cover(get_cover_id($vo["goodid"]),'path')); ?>"  width="70" height="70"/><?php echo (get_good_name($vo["goodid"])); ?></span><span><?php echo (get_good_name($vo["goodid"])); ?></span><span><?php echo (get_good_price($vo["goodid"])); ?></span></li><?php endforeach; endif; else: echo "" ;endif; ?></ul><?php endif; ?>
	</div>
...
//全选的实现
  ...
  function qiandao(){
var b='<?php echo U("center/enter");?>';
  $.ajax({
             type: "GET",
             url: "<?php echo U("center/enter");?>",
             data: {username:b},
             dataType: "json",
             success: function(data){
                        alert(data.msg);
