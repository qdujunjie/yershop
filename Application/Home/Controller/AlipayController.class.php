...
class PayController extends HomeController {
  ...
  public function _initialize() {
    ...
    // $Notify = new \Vendor\Notify();
	// echo  $Notify->MyFunction() . "\n";  
       if(is_login){
         $id=I("get.id");
		  $order=D("order");
		  $codeid=$order->where("id='$id'")->getField('orderid');
		 $this->assign('codeid',$codeid);
	  
	   $total=$order->where("id='$id'")->getField('pricetotal');
    ...
    public function doalipay(){
       ...
       $alipay_config=C('alipay_config');  
        ...
        $out_trade_no = $_POST['orderid'];//商户订单号 通过支付页面的表单进行传递，注意要唯一！
        $subject = C('SITENAME')."购物";  //订单名称 //必填 通过支付页面的表单进行传递
        $total_fee = $_POST['money'];   //付款金额  //必填 通过支付页面的表单进行传递
  ...
  $this->display('index');
    ...
    function returnurl(){
        ...
        $out_trade_no   = $_GET['out_trade_no'];     //商户网站订单系统中唯一订单号，必填
